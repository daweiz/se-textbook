...
\renewenvironment{codelisting}{
  \captionsetup{font=small,type=lstlisting}
}{}
...
\usepackage{etoolbox}
% pandoc>=3.8.2 对无题注表格输出 {\def\LTcaptype{none} ...}，旧版 longtable
% 需要该计数器存在，否则报 "No counter 'none' defined"。此处预定义之。
\newcounter{none}
...
\setsansfont[]{Arial}
\setmonofont[Consolas]{Courier New}
\setCJKmainfont[]{SimSun}
\setCJKsansfont[Microsoft YaHei]{SimHei}
%------------------------------------------------------------------------------
...
%\makeatletter
%\usepackage{listings}  % 依赖 listings 宏包
%\newenvironment{codelisting}[1][]{% 定义环境，支持参数
%%  \begin{lstlisting}[#1]
%%}{%
%%  \end{lstlisting}
%%}
...
\newenvironment{block}[1]{
  \begin{tcolorbox}[
    fontupper=\small\CJKfamily{SimSun}, 
    fonttitle=\large\bfseries\CJKfamily{SimSun}, % 标题也用SimSun（若有中文标题）
    colback=gray!10,    % 背景色（浅灰色，可修改，如 white!90 接近白色）
    colframe=third,     % 边框颜色（使用前面定义的 third 色）
    boxrule=0.1pt,        % 边框粗细（1pt，可调整）
    arc=1pt,            % 边框圆角（可选，0pt 为直角）
    left=1pt,right=1pt, % 左右内边距（避免内容贴边框）
    top=1pt,bottom=1pt  % 上下内边距
  ]
  % 原标题部分：加粗 + third 颜色
  % \textbf{\color{third}#1}\par % \par 确保标题后换行
}{%
  % 环境结束：关闭盒子
  \end{tcolorbox}
  %\par % 环境结束后换行
...
\usepackage{graphicx} % 必选：支持\includegraphics
\usepackage{float}       % 提供[H]选项（强制不浮动）
\usepackage{xpatch}     
\xpretocmd{\figure}{[h]}{}{}
...
Engineering{[}M{]}. Addison-Wesley, 1975.

{[}3{]} Brooks F P. No Silver Bullet: Essence and Accidents of Software
Engineering{[}J{]}. IEEE Computer, 1987, 20(4): 10-19.

{[}4{]} Bourque P, Dupuis R, Abran A, et al.~The Guide to the Software
Engineering Body of Knowledge{[}J{]}. IEEE Software, 1999: 35-44.

{[}5{]} Abran A, Moore J W. SWEBOK: Guide to the Software Engineering
Body of Knowledge{[}M{]}. IEEE Computer Society Press, 2002.

{[}6{]} IEEE/ACM Joint Task Force on Software Engineering Ethics and
Professional Practices. Software Engineering Code of Ethics and
Professional Practice{[}S{]}. 1999.

{[}7{]} Chen M, Tworek J, Jun H, et al.~Evaluating Large Language Models
Trained on Code{[}J{]}. arXiv:2107.03374, 2021.

{[}8{]} Peng S, Kalliamvakou E, Cihon P, et al.~The Impact of AI on
Developer Productivity: Evidence from GitHub Copilot{[}J{]}.
arXiv:2302.06590, 2023.

{[}9{]} IEEE Computer Society. Guide to the Software Engineering Body of
Knowledge (SWEBOK), Version 4.0{[}S{]}. 2024.

{[}10{]} Martínez-Fernández S, Bogner J, Franch X, et al.~Software
Engineering for AI-Based Systems: A Survey{[}J{]}. arXiv:2105.01984,
2021.

{[}11{]} Lewis P, Perez E, Piktus A, et al.~Retrieval-Augmented
Generation for Knowledge-Intensive NLP Tasks{[}C{]}. NeurIPS, 2020.

{[}12{]} Yao S, Zhao J, Yu D, et al.~ReAct: Synergizing Reasoning and
Acting in Language Models{[}C{]}. ICLR, 2023.

{[}13{]} Jimenez C E, Yang J, Wettig A, et al.~SWE-bench: Can Language
Models Resolve Real-World GitHub Issues?{[}C{]}. ICLR, 2024.

{[}14{]} Vaswani A, Shazeer N, Parmar N, et al.~Attention Is All You
Need{[}C{]}. NeurIPS, 2017.

{[}15{]} Li Y, Choi D, Chung J, et al.~Competition-Level Code Generation
with AlphaCode{[}J{]}. Science, 2022, 378(6624): 1092-1097.

{[}16{]} Yang J, Jimenez C E, Wettig A, et al.~SWE-agent: Agent-Computer
Interfaces Enable Automated Software Engineering{[}C{]}. NeurIPS, 2024.

{[}17{]} Wang L, Ma C, Feng X, et al.~A Survey on Large Language Model
based Autonomous Agents{[}J{]}. Frontiers of Computer Science, 2024,
18(6): 186345.

{[}18{]} Hassan A E, Oliva G A, Lin D, et al.~Towards AI-Native Software
Engineering (SE 3.0): A Vision and a Challenge Roadmap{[}J{]}. ACM
Transactions on Software Engineering and Methodology, 2024. arXiv:
2410.06107.

{[}19{]} Karpathy A. There's a New Kind of Coding Called ``Vibe
Coding''{[}EB/OL{]}. X (Twitter), 2025-02-02.

{[}20{]} Karpathy A. Agentic Engineering{[}EB/OL{]}. 2026-02.

{[}21{]} Dong Y, Jiang X, Qian J, et al.~A Survey on Code Generation
with LLM-based Agents{[}J{]}. arXiv: 2508.00083, 2025.
