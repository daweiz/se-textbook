...
function main() {
  ...
  if (!fs.existsSync(manifestFile)) {
    console.error('缺少清单 _manifest.js，请先运行 node docs/tools/generate.js');
    process.exit(1);
  }
  ...
  for (const entry of manifest) {
    let d;
    try {
      d = loadData(path.join(ROOT, 'docs', entry.file), 'SLIDES');
    } catch (e) {
  ...
  let expectTotal = 0;
  for (const entry of manifest) {
    const d = loadData(path.join(ROOT, 'docs', entry.file), 'SLIDES');
