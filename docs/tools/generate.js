...
//
// 用法：node docs/tools/generate.js
// 输出：
//   docs/data/chapter-NN.js   每章幻灯片数据（window.SLIDES，章节数据源）
//   docs/data/unit-NN.js      每小节讲解用 deck（window.SLIDES，按 tools/units.js 划分 + 拆细）
//   docs/data/_manifest.js    章节清单（window.CHAPTERS）+ 小节清单（window.UNITS）
//
...
function main() {
  ...
  for (const slug of slugs) {
    ...
    fs.writeFileSync(outFile, '// 由 docs/tools/generate.js 生成；旁白文本可人工润色\n', 'utf8');
  ...
  const chapterMap = {};
  for (const entry of manifest) chapterMap[entry.slug] = loadJsData(path.join(ROOT, 'docs', entry.file), 'SLIDES');
  ...
  for (const unitDef of units) {
    ...
    fs.writeFileSync(outFile, '// 由 docs/tools/generate.js 生成；封面旁白由 apply-narration.js 注入\n', 'utf8');
    fs.writeFileSync(outFile, 'window.SLIDES = ' + JSON.stringify(data, null, 2) + ';\n', 'utf8');
    unitManifest.push({
      num: data.unit,
      slug: data.slug,
