...
- **统计文件不参与 `/slides` 流水线**：`js/stats.js`、`stats.html`、`tools/stats-server.js` 为独立的可选统计能力（与生成/旁白/校验无关），运行 `/slides` 时不会被重建或覆盖；根目录 `stats-data/` 是统计服务的运行时数据（由 `tools/stats-server.js` 生成），不入库。改统计代码不涉及书稿，无需重跑 `/slides`。
