...
- **统计文件不参与 `/slides` 流水线**：`js/stats.js` 为独立的可选统计采集器（与生成/旁白/校验无关），运行 `/slides` 时不会被重建或覆盖。幻灯片行为统计由根目录 `server/`（`server/server.mjs` + `server/slides-stats.mjs`）提供，运行时数据存 `server/data/`（git 忽略）；原独立的 `tools/stats-server.js` 与报表页 `stats.html` 均已删除。改统计代码不涉及书稿，无需重跑 `/slides`。
