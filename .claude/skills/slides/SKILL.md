...
基于书稿 `src/chapter-NN/NNNN-*.md` 生成 / 重建幻灯片系统，输出到 `slides/`。全书按教学划分为 **37 个小节**（每节 60 分钟，是否含提问/交流环节见小节封面），**每节一套讲解用 deck**。幻灯片系统为**零依赖、纯前端、离线可用**的播放器，支持全屏播放、浏览器 TTS 语音朗读、字幕高亮与录屏下载。
...
- **37 节划分表**：`slides/tools/units.js` 是划分的单一事实来源——每节由 `parts: [{ch, from, to}]` 指定覆盖的章节内容片区间（下标 1 起，0 为章封面）、`minutes`（每节 60 分钟）、`qa`（提问/交流/讨论/演示等环节）。调整划分只改该文件再跑 `/slides`。
